...
const STORAGE_KEY = 'sideline-manager-v3-dynamic'
...
function halfLabel(halfNumber) {
  return Number(halfNumber) === 1 ? '1st Half' : '2nd Half'
}
...
function chooseDynamicSplit({ totalOnField, whiteCount, redCount, remainingWhiteTarget, remainingRedTarget, remainingBlocks }) {
  ...
  const redMajorityCandidates = candidates.filter((c) => c.redSpots > c.whiteSpots)
  const candidatePool = redMajorityCandidates.length ? redMajorityCandidates : candidates
...
function buildSchedule({ whitePlayers, redPlayers, totalOnField, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides }) {
  ...
  const spreadMinutes = summary.length ? Math.max(...summary.map((p) => p.minutes)) - Math.min(...summary.map((p) => p.minutes)) : 0
...
function toCsv(schedule, meta) {
  const intro = [
    ...
    ['Target minutes per player', meta.targetMinutesPerPlayer],
    ['Projected spread', meta.spreadMinutes],
    ['Elapsed game time', formatTime(meta.elapsedSeconds)],
    ['Active block', meta.currentBlockLabel],
    [],
  ]
  const header = ['Block title', 'Match Minutes', 'Split', 'White squad on', 'Red squad on']
  const rows = asArray(schedule.rows).map((row) => [row.label, row.matchLabel, row.splitLabel, asArray(row.onWhite).join(' | '), asArray(row.onRed).join(' | ')])
  const summaryHeader = ['Player', 'Squad', 'Blocks', 'Minutes']
  const summaryRows = asArray(schedule.summary).map((player) => [player.name, player.group, player.intervals, player.minutes])
  return [...intro, header, ...rows, [], summaryHeader, ...summaryRows].map((line) => line.map((cell) => `"${String(cell ?? '').replaceAll('"', '""')}"`).join(',')).join('\n')
}
...
function BrandMark({ small=false }) {
  return <img src="/icon.svg" alt="Sideline Manager logo" className={small ? 'brand-mark small' : 'brand-mark'} />
}
    ...
    <section className="card compact-meta no-print striped-card">
      <div className="meta-grid wide">
        ...
        <div className="meta-pill-group">
          <Badge tone="blue">Target: {targetMinutesPerPlayer.toFixed(1)} mins/player</Badge>
          <Badge tone={spreadMinutes <= 5 ? 'green' : 'amber'}>Projected spread: {spreadMinutes} mins</Badge>
          <Badge tone="red">Dynamic split mode</Badge>
        </div>
      </div>
    </section>
  )
...
function LandingPage({ onOpenManager, domainIdeas }) {
  const features = [
    'Dynamic whole-game fairness target',
    'White squad replaces White squad / Red replaces Red',
    'Current block swaps and future re-entry',
    'Sideline injury statuses and quick rebalancing',
  ]
  return (
    <div className="landing-shell">
      <section className="landing-hero striped-hero">
        <div className="landing-brand">
          ...
          <div>
            ...
            <h1>Sideline Manager</h1>
            <p>Built for coaches who need fair game time, quick interchange recovery, and a cleaner game-day view.</p>
          </div>
        </div>
        <div className="landing-cta-group">
          <button type="button" className="btn btn-primary" onClick={onOpenManager}>Open the manager</button>
          <div className="small muted light">A clean, shareable web app for substitution planning and live game adjustments.</div>
        </div>
      </section>
      ...
      <section className="landing-grid">
        ...
        <div className="card landing-card">
          <h2>Suggested domain ideas</h2>
          <ul>
            {domainIdeas.map((idea) => <li key={idea}><strong>{idea}</strong></li>)}
          </ul>
          <p className="small muted">These are ideas only — domain availability can change, so it is worth checking a registrar before locking one in.</p>
        </div>
      </section>

      <section className="card landing-card preview-card">
        <div className="row between center gap wrap">
          <div>
            <h2>Built for the real sideline</h2>
            <p className="muted">Red and white stripe branding, a proper icon, and a polished title screen make Sideline Manager feel like a real app instead of a spreadsheet on a phone.</p>
          </div>
          <BrandMark small />
        </div>
      </section>
  ...
  return (
    <section className="card live-console no-print striped-card">
      <div className="console-top">
        <div><div className="eyebrow">Current block</div><h1 className="console-block-title">{row.label}</h1><p className="muted small">Match minutes {row.matchLabel} · Split {row.splitLabel}</p></div>
        <div className="console-badges"><Badge tone={timerRunning ? 'green' : 'amber'}>{timerRunning ? 'Running' : 'Paused'}</Badge><Badge tone="red">{row.splitLabel}</Badge></div>
      </div>
      <div className="timer-strip">
        <div><div className="eyebrow">Game time</div><div className="timer-value">{formatTime(elapsedSeconds)}</div><div className="small muted">Remaining {formatTime(remaining)}</div></div>
        <div className="timer-actions-inline"><button type="button" className="btn btn-primary" onClick={onStartPause}>{timerRunning ? 'Pause' : 'Start'}</button><button type="button" className="btn btn-ghost" onClick={onResetTimer}>Reset</button></div>
      </div>
      ...
      <div className="console-middle">
        <div className="console-panel"><div className="group-title">White squad</div><ChipRow title="Go on now" names={row.incomingWhite} tone="green" emptyText={isStart ? 'Starting group' : 'No changes'} /><ChipRow title="Come off now" names={row.outgoingWhite} tone="red" emptyText={isStart ? 'Start of game' : 'No changes'} /></div>
        <div className="console-panel red-panel"><div className="group-title">Red squad</div><ChipRow title="Go on now" names={row.incomingRed} tone="green" emptyText={isStart ? 'Starting group' : 'No changes'} /><ChipRow title="Come off now" names={row.outgoingRed} tone="red" emptyText={isStart ? 'Start of game' : 'No changes'} /></div>
      </div>
...
function SquadStatusPanel({ squadLabel, players, unavailableSet, returningSet, markUnavailable, markAvailableAgain, resetStatuses }) {
  return (
    <section className="card">
      <div className="row between center gap wrap"><div><h2>{squadLabel} injury / return status</h2><div className="small muted">Mark players out for the rest of the game or bring them back into the pool.</div></div><button type="button" className="btn btn-ghost small-btn" onClick={resetStatuses}>Reset section</button></div>
      <div className="stack mt-sm">{asArray(players).map((name) => { const status = unavailableSet.has(name) ? 'Unavailable' : returningSet.has(name) ? 'Returning' : 'Available'; return <div key={`${squadLabel}-${name}`} className="status-row"><div className="status-left"><strong>{name}</strong><StatusChip status={status} /></div><div className="status-actions"><button type="button" className="btn btn-ghost small-btn" onClick={() => markUnavailable(name)}>Unavailable for rest of game</button><button type="button" className="btn btn-ghost small-btn" onClick={() => markAvailableAgain(name)}>Available again</button></div></div> })}</div>
    </section>
  )
}
...
function AvailabilityPanel({ title, players, absentSet, toggleAbsent, markAllPresent }) {
  return (
    <section className="card">
      <div className="row between start gap wrap"><div><h2>{title}</h2><p className="muted">Tap a player to mark them absent or present for this game.</p></div><button type="button" className="btn btn-ghost" onClick={markAllPresent}>Reset</button></div>
      <div className="chip-wrap mt">{asArray(players).map((player) => { const absent = absentSet.has(player); return <button key={player} type="button" className={`presence ${absent ? 'absent' : 'present'}`} onClick={() => toggleAbsent(player)}>{player} · {absent ? 'Absent' : 'Present'}</button> })}</div>
    </section>
  )
}
  ...
  return (
    <details className="game-tools no-print">
      ...
      <div className="stack mt-sm">
        <section className="card discreet-tools"><div className="tool-heading">Quick current block swap</div><p className="small muted">Swap out a player mid-block. Sideline Manager then rebuilds the remaining game using the updated fairness position.</p><div className="grid two-up mt-sm"><label><span>Squad</span><select value={currentSwapState.squad} onChange={(e) => setCurrentSwapState((c) => ({ ...c, squad: e.target.value, outgoing: '', incoming: '' }))}><option value="white">White squad</option><option value="red">Red squad</option></select></label><label><span>Player coming off now</span><select value={currentSwapState.outgoing} onChange={(e) => setCurrentSwapState((c) => ({ ...c, outgoing: e.target.value }))}><option value="">Choose player</option>{currentOnList.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="span-two"><span>Player going back on now</span><select value={currentSwapState.incoming} onChange={(e) => setCurrentSwapState((c) => ({ ...c, incoming: e.target.value }))}><option value="">Choose player</option>{currentBench.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div><button type="button" className="btn btn-ghost mt-sm" onClick={onApplyCurrentSwap}>Apply current block swap</button></section>
        <section className="card discreet-tools"><div className="tool-heading">Re-enter a player into an upcoming block</div><p className="small muted">Force a player back into a selected future block if that player is okay to return later.</p><div className="grid two-up mt-sm"><label><span>Squad</span><select value={futureState.squad} onChange={(e) => setFutureState((c) => ({ ...c, squad: e.target.value, player: '' }))}><option value="white">White squad</option><option value="red">Red squad</option></select></label><label><span>Upcoming block</span><select value={futureState.block} onChange={(e) => setFutureState((c) => ({ ...c, block: e.target.value }))}>{Array.from({ length: Math.max(totalBlocks, 0) }, (_, idx) => idx).filter((i) => i > 0).map((i) => <option key={i} value={String(i)}>{`Block ${i + 1}`}</option>)}</select></label><label className="span-two"><span>Player to force back in</span><select value={futureState.player} onChange={(e) => setFutureState((c) => ({ ...c, player: e.target.value }))}><option value="">Choose player</option>{futureOptions.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div><button type="button" className="btn btn-ghost mt-sm" onClick={onApplyFutureReturn}>Add to upcoming block</button></section>
        <section className="card discreet-tools"><div className="row between center gap wrap"><div><div className="tool-heading">Manual changes currently applied</div><div className="small muted">These changes are kept and the remaining blocks rebalance around them.</div></div><button type="button" className="btn btn-ghost small-btn" onClick={onClearAllOverrides}>Clear all</button></div><div className="stack mt-sm">{asArray(overrideItems).length ? asArray(overrideItems).map((item) => <div key={item.key} className="mini-card history-card"><div className="row between start gap wrap"><div><strong>{item.squadLabel} · {item.blockLabel}</strong><div className="small muted">On: {item.forceOn.join(', ') || 'None'} · Off: {item.forceOff.join(', ') || 'None'}</div></div><button type="button" className="btn btn-ghost small-btn" onClick={() => onClearOverride(item.squadLabel === 'White squad' ? 'white' : 'red', item.blockIndex)}>Remove</button></div></div>) : <div className="small muted">No manual changes saved yet.</div>}</div></section>
        <section className="card discreet-tools"><div className="tool-heading">New game page</div><p className="small muted">Archive this game, then start a fresh page for the next game week.</p><button type="button" className="btn btn-ghost" onClick={() => onArchiveAndStartNew(nextWeekIso(gameDate))}>Archive game & start next week</button><div className="small muted mt-sm">Saved previous games: {historyCount}</div></section>
        {children}
      </div>
    </details>
  )
...
export default function App() {
  ...
  const previousGame = useMemo(() => {
    const earlier = sortedHistory.filter((record) => String(record?.gameDate || '') < gameDate)
  ...
  const overrideItems = useMemo(() => {
    ...
    ;['white', 'red'].forEach((squadKey) => {
      ...
      Object.entries(blockMap).forEach(([blockText, override]) => {
        ...
        items.push({ key: `${squadKey}-${blockIndex}`, squadLabel: squadKey === 'white' ? 'White squad' : 'Red squad', blockIndex, blockLabel: row.label, forceOn: o.forceOn, forceOff: o.forceOff })
      })
    })
    return items.sort((a, b) => a.blockIndex - b.blockIndex || a.squadLabel.localeCompare(b.squadLabel))
  }, [overrides, schedule.rows])

  const domainIdeas = ['sidelinemanager.app', 'sidelinemanager.co.nz', 'playsidelinemanager.com', 'sidelineinterchange.com', 'redwhiterotation.com']
  ...
  useEffect(() => {
    ...
    const payload = {
      showLanding, gameDate, halfMinutes, intervalMinutes, totalOnField,
  ...
  const applyOverrideUpdate = (squadKey, blockIndex, updates) => {
    setOverrides((current) => {
      const safeCurrent = asObject(current, { white: {}, red: {} })
      const next = { ...safeCurrent, [squadKey]: { ...asObject(safeCurrent[squadKey], {}) } }
  ...
  const clearOverride = (squadKey, blockIndex) => setOverrides((current) => { const safeCurrent = asObject(current, { white:{}, red:{} }); const next = { ...safeCurrent, [squadKey]: { ...asObject(safeCurrent[squadKey], {}) } }; delete next[squadKey][String(blockIndex)]; return next })
  ...
  const archiveCurrentGame = (nextDate) => {
    const record = {
      ...
      elapsedSeconds,
      currentInterval,
      targetMinutesPerPlayer: schedule.targetMinutesPerPlayer,
      spreadMinutes: schedule.spreadMinutes,
    }
  ...
  const loadHistoricalGame = (recordId) => {
    ...
    setHalfMinutes(Number(selected.halfMinutes) || 25)
    setIntervalMinutes(Number(selected.intervalMinutes) || 5)
    setTotalOnField(Number(selected.totalOnField) || 9)
    setCurrentInterval(0); setElapsedSeconds(0); setTimerRunning(false)
    setOverrides({ white: {}, red: {} })
  ...
  if (showLanding) {
    return <div className="app-shell"><div className="container"><LandingPage onOpenManager={() => setShowLanding(false)} domainIdeas={domainIdeas} /></div></div>
  }

  return (
    <div className="app-shell">
      <div className="container">
        <section className="hero no-print compact-hero striped-hero"><div className="hero-brand"><BrandMark small /><div><h1>Sideline Manager</h1><p>Dynamic fairness, sideline recovery, and cleaner game-day decisions.</p></div></div><div className="hero-actions"><button type="button" className="btn btn-ghost" onClick={() => setShowLanding(true)}>Back to landing page</button></div></section>
        <TopMetaBar gameDate={gameDate} setGameDate={setGameDate} previousGameDate={previousGame?.gameDate || ''} targetMinutesPerPlayer={schedule.targetMinutesPerPlayer || 0} spreadMinutes={schedule.spreadMinutes || 0} />
        <div className="status-bar no-print">
          ...
          <Badge tone="red">Dynamic whole-game split</Badge>
          <Badge tone={schedule.spreadMinutes <= 5 ? 'green' : 'amber'}>Projected spread: {schedule.spreadMinutes} mins</Badge>
        </div>

        <div className="layout mobile-first-layout">
          <div className="sidebar no-print top-stack">
            <CompactLiveConsole row={liveRow} totalBlocks={asArray(schedule.rows).length} currentIndex={currentInterval} elapsedSeconds={elapsedSeconds} totalMatchSeconds={totalMatchSeconds} timerRunning={timerRunning} onStartPause={() => setTimerRunning((v) => !v)} onResetTimer={() => { setTimerRunning(false); setElapsedSeconds(0) }} onNext={() => setCurrentInterval((v) => Math.min(v + 1, maxIntervalIndex))} onBack={() => setCurrentInterval((v) => Math.max(v - 1, 0))} onSync={() => setCurrentInterval(timerBlockIndex)} timerBlockIndex={timerBlockIndex} mismatch={mismatch} />

            <GameTools gameDate={gameDate} historyCount={asArray(sortedHistory).length} onArchiveAndStartNew={archiveCurrentGame} currentSwapState={currentSwapState} setCurrentSwapState={setCurrentSwapState} futureState={futureState} setFutureState={setFutureState} currentRow={liveRow} whiteBench={whiteBench} redBench={redBench} totalBlocks={asArray(schedule.rows).length} onApplyCurrentSwap={onApplyCurrentSwap} onApplyFutureReturn={onApplyFutureReturn} overrideItems={overrideItems} onClearOverride={clearOverride} onClearAllOverrides={clearAllOverrides}>
              <section className="card">
                <div className="row between center gap wrap"><h2>Match setup</h2><Badge tone="red">Fairness first</Badge></div>
                <div className="grid three mt"><label><span>Minutes per half</span><input type="number" min="1" value={halfMinutes} onChange={(e) => setHalfMinutes(Number(e.target.value) || 0)} /></label><label><span>Sub interval</span><input type="number" min="1" value={intervalMinutes} onChange={(e) => setIntervalMinutes(Number(e.target.value) || 0)} /></label><label className="span-two"><span>Total players on field at one time</span><input type="number" min="1" max={Math.max(1, availableTotal || 1)} value={totalOnField} onChange={(e) => setTotalOnField(Number(e.target.value) || 0)} /></label></div>
                <div className="row wrap gap mt"><Badge tone="slate">Listed: {cleanWhite.length + cleanRed.length}</Badge><Badge tone="slate">Intervals: {intervals}</Badge><Badge tone="slate">Game: {totalMatchMinutes} mins</Badge><Badge tone="blue">Current block: {asArray(schedule.rows).length ? currentInterval + 1 : 0}/{asArray(schedule.rows).length}</Badge><Badge tone="blue">Timer block: {asArray(schedule.rows).length ? timerBlockIndex + 1 : 0}/{asArray(schedule.rows).length}</Badge></div>
                {!canGenerate ? <div className="alert mt"><strong>Please fix the setup</strong><ul>{halfMinutes % intervalMinutes !== 0 ? <li key='1'>Minutes per half must divide evenly by the sub interval.</li> : null}{totalOnField > availableTotal ? <li key='2'>Total players on field cannot exceed available players.</li> : null}{duplicateNames.length ? <li key='3'>Each player name must appear only once across both squads.</li> : null}{availableTotal === 0 ? <li key='4'>At least one player must be available.</li> : null}</ul></div> : null}
                <button type="button" className="btn btn-primary full mt" onClick={() => download('sideline-manager-schedule.csv', toCsv(schedule, { gameDate, totalOnField, whiteSpots: schedule.targetWhiteSlots, redSpots: schedule.targetRedSlots, elapsedSeconds, currentInterval, currentBlockLabel, targetMinutesPerPlayer: schedule.targetMinutesPerPlayer, spreadMinutes: schedule.spreadMinutes }), 'text/csv;charset=utf-8;')} disabled={!canGenerate}>Download CSV schedule</button>
              </section>

              <SquadStatusPanel squadLabel="White squad" players={cleanWhite} unavailableSet={unavailableSetWhite} returningSet={returningSetWhite} markUnavailable={(name) => markUnavailable('white', name)} markAvailableAgain={(name) => markAvailableAgain('white', name)} resetStatuses={() => { setUnavailableWhite([]); setReturningWhite([]) }} />
              <SquadStatusPanel squadLabel="Red squad" players={cleanRed} unavailableSet={unavailableSetRed} returningSet={returningSetRed} markUnavailable={(name) => markUnavailable('red', name)} markAvailableAgain={(name) => markAvailableAgain('red', name)} resetStatuses={() => { setUnavailableRed([]); setReturningRed([]) }} />

              <AvailabilityPanel title="White squad availability" players={cleanWhite} absentSet={absentSetWhite} toggleAbsent={(name) => setAbsentWhite((current) => asArray(current).includes(name) ? asArray(current).filter((value) => value !== name) : [...asArray(current), name])} markAllPresent={() => setAbsentWhite([])} />
              <AvailabilityPanel title="Red squad availability" players={cleanRed} absentSet={absentSetRed} toggleAbsent={(name) => setAbsentRed((current) => asArray(current).includes(name) ? asArray(current).filter((value) => value !== name) : [...asArray(current), name])} markAllPresent={() => setAbsentRed([])} />
          ...
          <div className="main">
            <MinutesSummary summary={summarySafe} totalMatchMinutes={totalMatchMinutes} intervalMinutes={intervalMinutes} targetMinutesPerPlayer={schedule.targetMinutesPerPlayer || 0} />
