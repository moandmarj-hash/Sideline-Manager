...
const STORAGE_KEY = 'sideline-manager-v4-collapsible'
...
function chooseDynamicSplit({ totalOnField, whiteCount, redCount, remainingWhiteTarget, remainingRedTarget, remainingBlocks }) {
  ...
  const preferred = candidates.filter((candidate) => candidate.redSpots > candidate.whiteSpots)
...
function pickPlayers(players, spots, equityScore, currentBlocks, lastPlayed, intervalIndex, cursor, override) {
  ...
  const rest = safePlayers
    ...
    .map((item) => item.name)
  const selected = [...forceOn, ...rest].sort((a, b) => safePlayers.indexOf(a) - safePlayers.indexOf(b))
...
function diff(previous, current) {
  ...
  return { incoming: cur.filter((name) => !prev.includes(name)), outgoing: prev.filter((name) => !cur.includes(name)), hasPrevious: true }
}
...
function buildSchedule({ whitePlayers, redPlayers, totalOnField, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides, completedIntervals }) {
  ...
  for (let intervalIndex = 0; intervalIndex < intervals; intervalIndex += 1) {
    const remainingBlocks = intervals - intervalIndex
    ...
    const start = intervalIndex * intervalMinutes
    const end = start + intervalMinutes
    const half = start < halfMinutes ? 1 : 2
    const whiteOverride = normalizeOverride(asObject(asObject(overrides).white, {})[String(intervalIndex)])
    const redOverride = normalizeOverride(asObject(asObject(overrides).red, {})[String(intervalIndex)])

    const pickWhite = pickPlayers(whitePlayers, split.whiteSpots, equityScore, currentBlocks, lastPlayed, intervalIndex, cursorWhite, whiteOverride)
    const pickRed = pickPlayers(redPlayers, split.redSpots, equityScore, currentBlocks, lastPlayed, intervalIndex, cursorRed, redOverride)
    ...
    const rowDiffWhite = diff(prevWhite, pickWhite.selected)
    const rowDiffRed = diff(prevRed, pickRed.selected)
    const status = intervalIndex < completedIntervals ? 'completed' : intervalIndex === completedIntervals ? 'current' : 'upcoming'
    ...
    rows.push({
      ...
      incomingWhite: rowDiffWhite.incoming,
      outgoingWhite: rowDiffWhite.outgoing,
      incomingRed: rowDiffRed.incoming,
      outgoingRed: rowDiffRed.outgoing,
      hasPrevious: rowDiffWhite.hasPrevious || rowDiffRed.hasPrevious,
      whiteOverride,
      redOverride,
      status,
    })
  ...
  const spreadMinutes = summary.length ? Math.max(...summary.map((entry) => entry.minutes)) - Math.min(...summary.map((entry) => entry.minutes)) : 0
...
function Disclosure({ title, children, defaultOpen = false }) {
  return <details className="disclosure no-print" open={defaultOpen}><summary>{title}</summary><div className="stack mt-sm">{children}</div></details>
}
      ...
      <section className="landing-grid">
        <div className="card landing-card">
          ...
          <ul>
            ...
            <li>Collapsible game-day sections for a cleaner phone view</li>
          </ul>
        </div>
        <div className="card landing-card">
...
function DateBar({ gameDate, setGameDate }) {
  return <section className="card compact-meta no-print"><label className="date-label solo"><span>Week / game date</span><input type="date" value={gameDate} onChange={(event) => setGameDate(event.target.value || todayIso())} /></label></section>
}
...
function QuickSwapSection({ currentSwapState, setCurrentSwapState, currentRow, whiteBench, redBench, onApplyCurrentSwap }) {
  const squad = currentSwapState.squad || 'white'
  const currentOnList = squad === 'white' ? asArray(currentRow?.onWhite) : asArray(currentRow?.onRed)
  const bench = squad === 'white' ? asArray(whiteBench) : asArray(redBench)
  return <section className="card discreet-tools"><div className="tool-heading">Quick current block swap</div><p className="small muted">Swap out a player mid-block. Sideline Manager rebalance is then projected from there.</p><div className="grid two-up mt-sm"><label><span>Squad</span><select value={currentSwapState.squad} onChange={(event) => setCurrentSwapState((current) => ({ ...current, squad: event.target.value, outgoing: '', incoming: '' }))}><option value="white">White squad</option><option value="red">Red squad</option></select></label><label><span>Player coming off now</span><select value={currentSwapState.outgoing} onChange={(event) => setCurrentSwapState((current) => ({ ...current, outgoing: event.target.value }))}><option value="">Choose player</option>{currentOnList.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label className="span-two"><span>Player going back on now</span><select value={currentSwapState.incoming} onChange={(event) => setCurrentSwapState((current) => ({ ...current, incoming: event.target.value }))}><option value="">Choose player</option>{bench.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div><button type="button" className="btn btn-ghost mt-sm" onClick={onApplyCurrentSwap}>Apply quick swap</button></section>
}

function ReEnterSection({ futureState, setFutureState, currentRow, whiteBench, redBench, totalBlocks, onApplyFutureReturn }) {
  const squad = futureState.squad || 'white'
  const futureOptions = squad === 'white' ? uniq([...asArray(currentRow?.onWhite), ...asArray(whiteBench)]) : uniq([...asArray(currentRow?.onRed), ...asArray(redBench)])
  return <section className="card discreet-tools"><div className="tool-heading">Re-enter a player into an upcoming block</div><p className="small muted">Force a player back into a selected future block if that player is okay to return later.</p><div className="grid two-up mt-sm"><label><span>Squad</span><select value={futureState.squad} onChange={(event) => setFutureState((current) => ({ ...current, squad: event.target.value, player: '' }))}><option value="white">White squad</option><option value="red">Red squad</option></select></label><label><span>Upcoming block</span><select value={futureState.block} onChange={(event) => setFutureState((current) => ({ ...current, block: event.target.value }))}>{Array.from({ length: Math.max(totalBlocks, 0) }, (_, index) => index).filter((value) => value > 0).map((value) => <option key={value} value={String(value)}>{`Block ${value + 1}`}</option>)}</select></label><label className="span-two"><span>Player to force back in</span><select value={futureState.player} onChange={(event) => setFutureState((current) => ({ ...current, player: event.target.value }))}><option value="">Choose player</option>{futureOptions.map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div><button type="button" className="btn btn-ghost mt-sm" onClick={onApplyFutureReturn}>Add returning player</button></section>
}
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setElapsedSeconds((current) => {
        const next = Math.min(current + 1, totalMatchSeconds)
  ...
  return (
    <div className="app-shell">
      <div className="container">
        <section className="hero no-print compact-hero striped-hero">
          <div className="hero-brand"><BrandMark /><div className="hero-copy"><div className="title-badge app"><h1>Sideline Manager</h1></div><GrassStrip /></div></div>
          <div className="hero-actions"><button type="button" className="btn btn-ghost tiny-btn" onClick={() => setShowLanding(true)}>Back to landing</button></div>
        </section>

        <DateBar gameDate={gameDate} setGameDate={setGameDate} />
        ...
        <div className="layout mobile-first-layout">
          <div className="sidebar no-print top-stack">
            ...
            <Disclosure title="Quick Swap">
              <QuickSwapSection currentSwapState={currentSwapState} setCurrentSwapState={setCurrentSwapState} currentRow={liveRow} whiteBench={whiteBench} redBench={redBench} onApplyCurrentSwap={onApplyCurrentSwap} />
            </Disclosure>

            <Disclosure title="Re-enter a player">
              <ReEnterSection futureState={futureState} setFutureState={setFutureState} currentRow={liveRow} whiteBench={whiteBench} redBench={redBench} totalBlocks={asArray(schedule.rows).length} onApplyFutureReturn={onApplyFutureReturn} />
            </Disclosure>

            <Disclosure title="Game Tools">
              <GameTools gameDate={gameDate} historyCount={asArray(sortedHistory).length} onArchiveAndStartNew={archiveCurrentGame} overrideItems={overrideItems} onClearOverride={clearOverride} onClearAllOverrides={clearAllOverrides}>
                ...
                <section className="card">
                  ...
                  <div className="grid three mt"><label><span>Minutes per half</span><input type="number" min="1" value={halfMinutes} onChange={(event) => setHalfMinutes(Number(event.target.value) || 0)} /></label><label><span>Sub interval</span><input type="number" min="1" value={intervalMinutes} onChange={(event) => setIntervalMinutes(Number(event.target.value) || 0)} /></label><label className="span-two"><span>Total players on field at one time</span><input type="number" min="1" max={Math.max(1, availableTotal || 1)} value={totalOnField} onChange={(event) => setTotalOnField(Number(event.target.value) || 0)} /></label></div>
                  {!canGenerate ? <div className="alert mt"><strong>Please fix the setup</strong><ul>{halfMinutes % intervalMinutes !== 0 ? <li key="a">Minutes per half must divide evenly by the sub interval.</li> : null}{totalOnField > availableTotal ? <li key="b">Total players on field cannot exceed available players.</li> : null}{duplicateNames.length ? <li key="c">Each player name must appear only once across both squads.</li> : null}{availableTotal === 0 ? <li key="d">At least one player must be available.</li> : null}</ul></div> : null}
                </section>
            ...
            <Disclosure title="Minutes Summary" defaultOpen>
              <MinutesSummary summary={summarySafe} totalMatchMinutes={totalMatchMinutes} intervalMinutes={intervalMinutes} targetMinutesPerPlayer={schedule.targetMinutesPerPlayer || 0} spreadMinutes={schedule.spreadMinutes || 0} availableCount={availableTotal} totalOnField={totalOnField} previousGameDate={previousGame?.gameDate || ''} />
            </Disclosure>

            <Disclosure title="Rotation Plan" defaultOpen>
              <RotationPlan rows={schedule.rows} currentInterval={currentInterval} />
            </Disclosure>

            <Disclosure title="Previous game records">
              <HistoryPanel history={sortedHistory} onLoadGame={loadHistoricalGame} />
            </Disclosure>
          </div>

          <div className="main no-print">
            <div className="card helper-card"><strong>Projected minutes now reflect the current plan.</strong><div className="small muted mt-xs">When an injury, quick swap, unavailable status, or re-entry change is made mid-game, Sideline Manager recalculates the remaining blocks and the minutes summary updates to show projected final minutes from that new state.</div></div>
          </div>
        </div>
      </div>
    </div>
