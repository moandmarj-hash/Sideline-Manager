...
const STORAGE_KEY = 'sideline-manager-v2-failsafe'
...
function halfLabel(half) {
  return Number(half) === 1 ? '1st Half' : '2nd Half'
}
...
function loadState() {
  ...
  try {
    ...
    return {
      ...
      overrides: {
        white: asObject(overrides.white, {}),
        red: asObject(overrides.red, {}),
      },
...
function findBestSplit(totalOnField, whiteCount, redCount, blockCount, intervalMinutes) {
  let best = null
  for (let whiteSpots = 0; whiteSpots <= Math.min(Number(totalOnField) || 0, Number(whiteCount) || 0); whiteSpots += 1) {
    const redSpots = (Number(totalOnField) || 0) - whiteSpots
    if (redSpots < 0 || redSpots > (Number(redCount) || 0)) continue
...
function pickPlayers(players, spots, previousCredit, currentBlocks, lastPlayed, intervalIndex, cursor, override) {
  ...
  const remaining = safePlayers
    ...
    .map((name) => ({
      ...
      prevCredit: Number(previousCredit[name]) || 0,
  ...
  selected.forEach((name) => {
    previousCredit[name] = (Number(previousCredit[name]) || 0) + 1
...
function buildSchedule({ whitePlayers, redPlayers, whiteSpots, redSpots, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides }) {
  ...
  const currentBlocks = {}
  const lastPlayed = {}
  ;[...asArray(whitePlayers), ...asArray(redPlayers)].forEach((name) => {
    previousCredit[name] = (Number(priorMinutes[name]) || 0) / Math.max(1, intervalMinutes)
  ...
  for (let i = 0; i < intervals; i += 1) {
    ...
    const wOverride = normalizeOverride(asObject(asObject(overrides).white, {})[String(i)])
    const rOverride = normalizeOverride(asObject(asObject(overrides).red, {})[String(i)])
  ...
  const summary = [...asArray(whitePlayers), ...asArray(redPlayers)].map((name) => ({
    name,
  ...
  return (
    <section className="card live-console no-print">
      <div className="console-top">
        <div><div className="eyebrow">Current block</div><h1 className="console-block-title">{row.label}</h1><p className="muted small">Match minutes {row.matchLabel}</p></div>
        <div className="console-badges"><Badge tone={timerRunning ? 'green' : 'amber'}>{timerRunning ? 'Running' : 'Paused'}</Badge><Badge tone={mismatch ? 'amber' : 'blue'}>Timer block {timerBlockIndex + 1}</Badge></div>
      </div>
      ...
      <div className="console-middle">
        ...
        <div className="console-panel"><div className="group-title">Red squad</div><ChipRow title="Go on now" names={row.incomingRed} tone="green" emptyText={isStart ? 'Starting group' : 'No changes'} /><ChipRow title="Come off now" names={row.outgoingRed} tone="red" emptyText={isStart ? 'Start of game' : 'No changes'} /></div>
      </div>
...
function AvailabilityPanel({ title, players, absentSet, toggleAbsent, markAllPresent }) {
  return (
    <section className="card">
      <div className="row between start gap"><div><h2>{title}</h2><p className="muted">Tap a player to mark them absent or present.</p></div><button type="button" className="btn btn-ghost" onClick={markAllPresent}>Reset</button></div>
...
function MinutesSummary({ summary, totalMatchMinutes, intervalMinutes }) {
  return (
    <section className="card no-print">
      ...
      <div className="grid summary-grid mt">{asArray(summary).map((player) => { const isRedSquad = player.group === 'Red squad'; return <div key={player.name} className={`mini-card minutes-card ${isRedSquad ? 'red-squad-tile' : 'white-squad-tile'}`}><div className="row between start gap"><div><strong>{player.name}</strong><div className="small muted">{player.group}</div></div><Badge tone={isRedSquad ? 'red' : 'slate'}>{player.minutes} mins</Badge></div><div className="progress mt-sm"><div className={isRedSquad ? 'progress-red' : ''} style={{ width: `${Math.max(8, totalMatchMinutes ? (player.minutes / totalMatchMinutes) * 100 : 0)}%` }} /></div><div className="small muted mt-sm">{player.intervals} × {intervalMinutes}-minute blocks</div></div> })}</div>
    </section>
  )
}
...
function RotationPlan({ rows, currentInterval }) {
  return (
    <section className="card no-print">
      <div className="row between center gap"><div><h2>Rotation plan</h2><p className="muted">Completed blocks are greyed and current block is highlighted.</p></div><Badge tone="slate">{asArray(rows).length} blocks</Badge></div>
      <div className="stack mt">{asArray(rows).map((row) => { const status = row.intervalIndex < currentInterval ? 'completed' : row.intervalIndex === currentInterval ? 'current' : 'upcoming'; const statusLabel = status === 'completed' ? 'Done' : status === 'current' ? 'Current' : 'Up next'; const statusTone = status === 'completed' ? 'slate' : status === 'current' ? 'green' : 'blue'; return <div key={row.key} className={`mini-card block-card ${status}`}><div className="row between start gap"><div><h3>{row.label}</h3><div className="small muted">Match minutes {row.matchLabel}</div></div><div className="column gap-xs align-end"><Badge tone={statusTone}>{statusLabel}</Badge><Badge tone="blue">Block {row.intervalIndex + 1}</Badge></div></div><div className="stack mt-sm"><div className="subcard"><h3>White squad</h3><ChipRow title={!row.hasPrevious ? 'Starting on' : 'On now'} names={row.onWhite} tone="blue" emptyText="None" /></div><div className="subcard"><h3>Red squad</h3><ChipRow title={!row.hasPrevious ? 'Starting on' : 'On now'} names={row.onRed} tone="blue" emptyText="None" /></div></div></div> })}</div>
    </section>
  )
}

function HistoryPanel({ history, onLoadGame }) {
  return <details className="history-panel no-print"><summary>Previous game records ({asArray(history).length})</summary><div className="stack mt-sm">{asArray(history).length ? asArray(history).map((record) => <div key={record.id} className="mini-card history-card"><div className="row between start gap"><div><strong>{record.gameDate}</strong><div className="small muted">White avg {record.perPlayerMinutesWhite?.toFixed?.(1) ?? '0'} mins · Red avg {record.perPlayerMinutesRed?.toFixed?.(1) ?? '0'} mins</div></div><button type="button" className="btn btn-ghost" onClick={() => onLoadGame(record.id)}>Load</button></div></div>) : <div className="card"><div className="small muted">No previous games saved yet.</div></div>}</div></details>
}
...
export default function App() {
  ...
  const schedule = useMemo(() => buildSchedule({ whitePlayers: availableWhite, redPlayers: availableRed, whiteSpots, redSpots, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides }), [availableWhite, availableRed, whiteSpots, redSpots, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides])
  ...
  const onApplyFutureReturn = () => {
    ...
    if (futureState.squad === 'white') {
      setUnavailableWhite((cur) => asArray(cur).filter((name) => name !== futureState.player))
      setReturningWhite((cur) => uniq([...asArray(cur), futureState.player]))
    } else {
      setUnavailableRed((cur) => asArray(cur).filter((name) => name !== futureState.player))
      setReturningRed((cur) => uniq([...asArray(cur), futureState.player]))
    }
    setFutureState((c) => ({ ...c, player: '' }))
  }
  const markUnavailable = (squadKey, player) => {
    if (squadKey === 'white') {
      setUnavailableWhite((cur) => uniq([...asArray(cur), player]))
      setReturningWhite((cur) => asArray(cur).filter((name) => name !== player))
      if (asArray(liveRow?.onWhite).includes(player)) applyOverrideUpdate('white', currentInterval, { forceOff: [player] })
    } else {
      setUnavailableRed((cur) => uniq([...asArray(cur), player]))
      setReturningRed((cur) => asArray(cur).filter((name) => name !== player))
  ...
  const markAvailableAgain = (squadKey, player) => {
    if (squadKey === 'white') {
      setUnavailableWhite((cur) => asArray(cur).filter((name) => name !== player))
      setReturningWhite((cur) => uniq([...asArray(cur), player]))
    } else {
      setUnavailableRed((cur) => asArray(cur).filter((name) => name !== player))
      setReturningRed((cur) => uniq([...asArray(cur), player]))
    }
  }
  const archiveCurrentGame = (nextDate) => {
    ...
    setAbsentWhite([]); setAbsentRed([]); setUnavailableWhite([]); setUnavailableRed([]); setReturningWhite([]); setReturningRed([])
    setCurrentInterval(0); setElapsedSeconds(0); setTimerRunning(false); setOverrides({ white: {}, red: {} })
  ...
  const loadHistoricalGame = (recordId) => {
    ...
    setCurrentInterval(0); setElapsedSeconds(0); setTimerRunning(false); setOverrides({ white: {}, red: {} }); setUnavailableWhite([]); setUnavailableRed([]); setReturningWhite([]); setReturningRed([])
  }

  return (
    <div className="app-shell">
      <div className="container">
        <section className="hero no-print compact-hero"><h1>Sideline Manager</h1><p>Built for quick sideline use.</p></section>
        ...
        <div className="layout mobile-first-layout">
          <div className="sidebar no-print top-stack">
            ...
            <GameTools gameDate={gameDate} historyCount={asArray(sortedHistory).length} onArchiveAndStartNew={archiveCurrentGame} currentSwapState={currentSwapState} setCurrentSwapState={setCurrentSwapState} futureState={futureState} setFutureState={setFutureState} currentRow={liveRow} whiteBench={whiteBench} redBench={redBench} totalBlocks={asArray(schedule.rows).length} onApplyCurrentSwap={onApplyCurrentSwap} onApplyFutureReturn={onApplyFutureReturn} overrideItems={overrideItems} onClearOverride={clearOverride} onClearAllOverrides={clearAllOverrides}>
              <section className="card"><h2>Match setup</h2><div className="grid three mt"><label><span>Minutes per half</span><input type="number" min="1" value={halfMinutes} onChange={(e) => setHalfMinutes(Number(e.target.value) || 0)} /></label><label><span>Sub interval</span><input type="number" min="1" value={intervalMinutes} onChange={(e) => setIntervalMinutes(Number(e.target.value) || 0)} /></label><label className="span-two"><span>Total players on field at one time</span><input type="number" min="1" max={Math.max(1, availableTotal || 1)} value={totalOnField} onChange={(e) => setTotalOnField(Number(e.target.value) || 0)} /></label></div><div className="row wrap gap mt"><Badge tone="slate">Listed: {cleanWhite.length + cleanRed.length}</Badge><Badge tone="slate">Intervals: {intervals}</Badge><Badge tone="slate">Game: {totalMatchMinutes} mins</Badge><Badge tone="blue">Current block: {asArray(schedule.rows).length ? currentInterval + 1 : 0}/{asArray(schedule.rows).length}</Badge><Badge tone="blue">Timer block: {asArray(schedule.rows).length ? timerBlockIndex + 1 : 0}/{asArray(schedule.rows).length}</Badge></div>{!canGenerate ? <div className="alert mt"><strong>Please fix the setup</strong><ul>{halfMinutes % intervalMinutes !== 0 ? <li key='1'>Minutes per half must divide evenly by the sub interval.</li> : null}{totalOnField > availableTotal ? <li key='2'>Total players on field cannot exceed available players.</li> : null}{duplicateNames.length ? <li key='3'>Each player name must appear only once across both squads.</li> : null}{!split ? <li key='4'>No valid split could be found with the current available-player setting.</li> : null}</ul></div> : null}<button type="button" className="btn btn-primary full mt" onClick={() => download('sideline-manager-schedule.csv', toCsv(schedule, { gameDate, totalOnField, whiteSpots, redSpots, absentWhite: Array.from(absentSetWhite), absentRed: Array.from(absentSetRed), elapsedSeconds, currentInterval, currentBlockLabel }), 'text/csv;charset=utf-8;')} disabled={!canGenerate}>Download CSV schedule</button></section>
              ...
              <AvailabilityPanel title="White squad availability" players={cleanWhite} absentSet={absentSetWhite} toggleAbsent={(name) => setAbsentWhite((cur) => asArray(cur).includes(name) ? asArray(cur).filter((v) => v !== name) : [...asArray(cur), name])} markAllPresent={() => setAbsentWhite([])} />
              <AvailabilityPanel title="Red squad availability" players={cleanRed} absentSet={absentSetRed} toggleAbsent={(name) => setAbsentRed((cur) => asArray(cur).includes(name) ? asArray(cur).filter((v) => v !== name) : [...asArray(cur), name])} markAllPresent={() => setAbsentRed([])} />
          ...
          <div className="main"><MinutesSummary summary={summarySafe} totalMatchMinutes={totalMatchMinutes} intervalMinutes={intervalMinutes} /><RotationPlan rows={schedule.rows} currentInterval={currentInterval} /></div>
        </div>
      </div>
    </div>
  )
