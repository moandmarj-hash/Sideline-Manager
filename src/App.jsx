...
const STORAGE_KEY = 'sideline-manager-v4-1-final'
...
function chooseDynamicSplit({ totalOnField, whiteCount, redCount, remainingWhiteTarget, remainingRedTarget, remainingBlocks }) {
  const all=[]
  for (let w=0; w<=Math.min(totalOnField, whiteCount); w+=1) { const r=totalOnField-w; if (r>=0 && r<=redCount) all.push({whiteSpots:w, redSpots:r}) }
  if (!all.length) return { whiteSpots:0, redSpots:0 }
...
function buildSchedule({ whitePlayers, redPlayers, totalOnField, intervals, intervalMinutes, halfMinutes, priorMinutes, overrides, completedIntervals }) {
  ...
  for(let i=0;i<intervals;i+=1){
    const split=chooseDynamicSplit({ totalOnField, whiteCount:asArray(whitePlayers).length, redCount:asArray(redPlayers).length, remainingWhiteTarget:Math.max(0,targetWhiteSlots-assignedWhite), remainingRedTarget:Math.max(0,targetRedSlots-assignedRed), remainingBlocks:intervals-i })
  ...
  return { rows, summary, targetMinutesPerPlayer, spreadMinutes, targetWhiteSlots, targetRedSlots }
}

function BrandMark({ landing=false }) { return <img src="/icon.svg" alt="Sideline Manager logo" className={`brand-mark ${landing ? 'landing-size' : ''}`} /> }
...
function LandingPage({ onOpenManager, domainIdeas }) {
  return <div className="landing-shell"><section className="landing-hero striped-hero"><div className="landing-brand"><BrandMark landing /><div className="landing-copy"><div className="eyebrow light">Sideline-ready rotation control</div><div className="title-badge landing"><h1>Sideline Manager</h1></div><GrassStrip large /></div></div><div className="landing-cta-group"><button type="button" className="btn btn-primary" onClick={onOpenManager}>Open the manager</button><div className="small muted light">Dynamic fairness, smart split selection, and live sideline recovery in one place.</div></div></section><section className="landing-grid"><div className="card landing-card"><h2>What this build focuses on</h2><ul><li>Whole-game fairness target across all available players</li><li>Dynamic White / Red split by block</li><li>Rebalance after injuries and manual changes</li><li>Projected minutes update as the remaining game is rebuilt after changes</li></ul></div><div className="card landing-card"><h2>Shareable domain ideas</h2><ul>{domainIdeas.map((idea)=><li key={idea}><strong>{idea}</strong></li>)}</ul><p className="small muted">My pick: <strong>sidelinemanager.app</strong></p></div></section></div>
}
...
function CompactLiveConsole({ row, totalBlocks, currentIndex, elapsedSeconds, totalMatchSeconds, timerRunning, onStartPause, onResetTimer, onNext, onBack, onSync, timerBlockIndex, mismatch }) {
  ...
  return <section className="card live-console no-print striped-card"><div className="console-top"><div><div className="eyebrow">Current block</div><h1 className="console-block-title">{row.label}</h1><p className="muted small">Match minutes {row.matchLabel} · Split {row.splitLabel}</p></div><div className="console-badges"><Badge tone={timerRunning?'green':'amber'}>{timerRunning?'Running':'Paused'}</Badge><Badge tone="red">{row.splitLabel}</Badge></div></div><div className="timer-strip"><div><div className="eyebrow">Game time</div><div className="timer-value">{formatTime(elapsedSeconds)}</div><div className="small muted">Remaining {formatTime(remaining)}</div></div><div className="timer-actions-inline"><button type="button" className="btn btn-primary" onClick={onStartPause}>{timerRunning?'Pause':'Start'}</button><button type="button" className="btn btn-ghost" onClick={onResetTimer}>Reset</button></div></div><div className="progress mt-sm"><div style={{width:`${progress}%`}} /></div>{mismatch ? <div className="alert alert-amber mt-sm compact-alert"><strong>Out of sync:</strong> timer says block {timerBlockIndex+1}. <button type="button" className="btn btn-warning mt-xs full" onClick={onSync}>Sync to timer block</button></div> : null}<div className="console-middle"><div className="console-panel"><div className="group-title">White squad</div><ChipRow title="Go on now" names={row.incomingWhite} tone="green" emptyText={isStart?'Starting group':'No changes'} /><ChipRow title="Come off now" names={row.outgoingWhite} tone="red" emptyText={isStart?'Start of game':'No changes'} /></div><div className="console-panel red-panel"><div className="group-title">Red squad</div><ChipRow title="Go on now" names={row.incomingRed} tone="green" emptyText={isStart?'Starting group':'No changes'} /><ChipRow title="Come off now" names={row.outgoingRed} tone="red" emptyText={isStart?'Start of game':'No changes'} /></div></div><div className="console-bottom"><button type="button" className="btn btn-ghost full" onClick={onBack} disabled={atStart}>Previous</button><button type="button" className="btn btn-primary full next-button" onClick={onNext} disabled={atEnd}>{atEnd?'Last block':'Next interval'}</button></div></section>
}

function QuickSwapSection({ currentSwapState, setCurrentSwapState, currentRow, whiteBench, redBench, onApplyCurrentSwap }) {
  const squad=currentSwapState.squad||'white', onList=squad==='white'?asArray(currentRow?.onWhite):asArray(currentRow?.onRed), bench=squad==='white'?asArray(whiteBench):asArray(redBench)
  return <section className="card discreet-tools"><div className="tool-heading">Quick current block swap</div><p className="small muted">Swap out a player mid-block. Sideline Manager rebalance is then projected from there.</p><div className="grid two-up mt-sm"><label><span>Squad</span><select value={currentSwapState.squad} onChange={(e)=>setCurrentSwapState((c)=>({...c,squad:e.target.value,outgoing:'',incoming:''}))}><option value="white">White squad</option><option value="red">Red squad</option></select></label><label><span>Player coming off now</span><select value={currentSwapState.outgoing} onChange={(e)=>setCurrentSwapState((c)=>({...c,outgoing:e.target.value}))}><option value="">Choose player</option>{onList.map((n)=><option key={n} value={n}>{n}</option>)}</select></label><label className="span-two"><span>Player going back on now</span><select value={currentSwapState.incoming} onChange={(e)=>setCurrentSwapState((c)=>({...c,incoming:e.target.value}))}><option value="">Choose player</option>{bench.map((n)=><option key={n} value={n}>{n}</option>)}</select></label></div><button type="button" className="btn btn-ghost mt-sm" onClick={onApplyCurrentSwap}>Apply quick swap</button></section>
}
...
export default function App() {
  ...
  const onApplyCurrentSwap=()=>{ if(!currentSwapState.outgoing || !currentSwapState.incoming) return; applyOverrideUpdate(currentSwapState.squad, currentInterval, { forceOff:[currentSwapState.outgoing], forceOn:[currentSwapState.incoming] }); setCurrentSwapState((c)=>({ ...c, outgoing:'', incoming:'' })) }
